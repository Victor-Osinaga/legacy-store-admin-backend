class Client {
  ...
  setPassword(password) {
    if (
      ...
      throw { msg: "PASSWORD es requerida" };
  ...
  setSubdomain(subdomain) {
    if (!subdomain || subdomain.length == 0 || subdomain.trim() == "")
      throw { msg: "SUBDOMAIN es requerida" };
  ...
  setProyectName(proyectName) {
    if (!proyectName || proyectName.length == 0 || proyectName.trim() == "")
      throw { msg: "PROYECTNAME es requerida" };
  ...
  setLastname(lastname) {
    if (!lastname || lastname.length == 0 || lastname.trim() == "")
      throw { msg: "LASTNAME es requerida", status: 400 };
